...
// PATCH (branch tier2-esp-nn-simd): route FULLY_CONNECTED to esp-nn's reference
// kernel — optimized S3 kernels miscompute on this toolchain (see depthwise). Tiny
// op, no speed cost; convs keep SIMD. Regenerated by any SDK redeploy.
#undef esp_nn_fully_connected_s8
    ...
    case kTfLiteInt8: {
      ...
      const int32_t* bias_data =
          nullptr != bias ? tflite::micro::GetTensorData<int32_t>(bias)
                          : nullptr;
#if 0  // PATCH (tier2-esp-nn-simd): route FULLY_CONNECTED to TFLite reference (#else)
       // — esp-nn opt==ansi but both differ from reference on this toolchain.
      const RuntimeShape& filter_shape = tflite::micro::GetTensorShape(filter);
      const RuntimeShape& output_shape = tflite::micro::GetTensorShape(output);
      const int filter_dim_count = filter_shape.DimensionsCount();
      const int batches = output_shape.Dims(0);
