...
// PATCH (branch tier2-esp-nn-simd): esp-nn's OPTIMIZED depthwise S3 kernel
// miscomputes on this board/toolchain (self-test: 1008/1024 outputs wrong), while
// the regular conv kernel is bit-exact. Force ONLY the depthwise op to esp-nn's
// correct C reference (_ansi); conv and every other op keep the working SIMD path.
// MobileNet's MACs are dominated by 1x1 pointwise convs (regular conv, accelerated),
// so this stays correct AND keeps most of the speedup. Re-evaluate if esp-nn or the
// toolchain fixes the S3 depthwise kernel. NOTE: regenerated by any SDK redeploy.
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {  // Already know in/out types are same.
    ...
    case kTfLiteInt8:
      ...
      return kTfLiteError;
#endif
#if ESP_NN  // depthwise uses esp-nn _ansi (remapped at top of file): the OPTIMIZED
            // S3 depthwise kernel miscomputes on this toolchain, but _ansi is correct
            // here and faster than TFLite reference C. Set to `#if 0` to fall back to
            // TFLite reference if a redeploy/upgrade ever regresses this.
