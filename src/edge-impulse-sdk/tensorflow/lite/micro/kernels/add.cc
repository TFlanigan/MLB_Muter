...
// PATCH (branch tier2-esp-nn-simd): route ADD to esp-nn's reference kernel — the
// optimized S3 kernels miscompute on this toolchain (see depthwise). Tiny op, no
// speed cost; convs keep SIMD. Regenerated by any SDK redeploy.
#undef esp_nn_add_elementwise_s8
...
#if 0  // PATCH (tier2-esp-nn-simd): route ADD to TFLite reference (#else) — esp-nn
       // opt==ansi but both differ from reference on this toolchain, biasing the model.
